    ...
    function attachProductCardListeners() {
        document.querySelectorAll('.product-card').forEach(card => {
            card.addEventListener('click', () => {
                ...
                document.querySelector('#modalProductImage').src = product.image || 'default-image.png';
                ...
                document.querySelector('#modalProductTrangThai').textContent =
                    product.trangThai === '1' || product.trangThai === 1 ? 'Còn hàng' : 'Hết hàng';

                // Thumbnail gallery
                const thumbnailHTML = `
                    <img src="${product.image || 'default-image.png'}" alt="${product.name} thumbnail" class="thumbnail-image active" data-index="0">
                `;
