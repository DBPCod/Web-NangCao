...
class CTQuyenModel {
    ...
    public function addCtQuyen($data) {
        $stmt = $this->db->prepare("INSERT INTO ctquyen (IdQuyen, IdVaiTro, TrangThai) VALUES (?, ?, ?)");
        $stmt->bind_param("iii", $data['IdQuyen'], $data['IdVaiTro'], $data['TrangThai']);
        $stmt->execute();
        return $this->getCtQuyenById($data['IdQuyen'], $data['IdVaiTro']);
    }
