  ...
  -- Tạo bảng NguoiDung (Người dùng)
    ...
    IdKhuyenMai INT,
    IdCHSP INT,
    IdDongSanPham VARCHAR(20), -- Sửa từ INT thành VARCHAR(20)
    PRIMARY KEY (IdKhuyenMai, IdCHSP, IdDongSanPham),
    FOREIGN KEY (IdKhuyenMai) REFERENCES KhuyenMai(IdKhuyenMai),
    FOREIGN KEY (IdCHSP) REFERENCES SanPham(IdCHSP),
    FOREIGN KEY (IdDongSanPham) REFERENCES SanPham(IdDongSanPham) -- Tham chiếu đúng bảng
);
